...
namespace Horseshoe.NET.ObjectsAndTypes
{
    ...
    public static class ObjectUtil
    {
        ...
        public static void MapProperties(object src, object dest, bool tryMapAll = false, bool preventNullOverwrite = false, bool ignoreCase = false, bool ignoreErrors = false)
        {
            ...
            if (srcType == destType)
            {
                var srcPropValues = GetInstancePropertyValues(src)
                    .Where(pv => pv.Property.CanRead && pv.Property.CanWrite)
                    .ToArray();
            ...
            {
                var srcPropValues = GetInstancePropertyValues(src)
                    .Where(pv => pv.Property.CanRead)
                    .ToArray();
                var destProps = TypeUtil.GetInstanceProperties(destType)
                    .Where(p => p.CanWrite)
                    .ToArray();
